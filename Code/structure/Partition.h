...
public:
    ...
    bool set_ext_pts();
    bool set_ext_pts(point_t *feasible_pt);
    ...
    void prune_points(point_set *pset, int k, std::vector<std::string> &category, std::vector<std::vector<double>> &bound);

    bool divide(hyperplane *h, Partition *ptSet, Partition *ngSet);
    double largest_utility(point_t *u);
};
